<link rel="stylesheet" href="styles.css">
<img width=100% src="https://capsule-render.vercel.app/api?type=waving&color=b703bc&height=120&section=header"/>

### Who am I?

Hello! My name is Henrique Ângelo Censi de Andrade, you can call me Ângelo.
I'm passionate about back-end development and enjoy using my creativity to connect technologies and develop smart, high-performance solutions.

**My main technologies:**

<div class="skills">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="40" height="40" alt="JavaScript icon"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" width="40" height="40" alt="Node icon"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/csharp/csharp-original.svg" width="40" height="40" alt="C# icon"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="40" height="40" alt="HTML5 icon"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="40" height="40" alt="CSS3 icon"/>
</div>

---

### Projects:

<a href="https://github.com/Angelo-Andrade/ticket-flow" target="_blank">
  Ticket Flow
</a>

<p>
  Ticket Flow is a MVC web application built with Node.js, Express, and MySQL, designed to manage IT trouble tickets for a Brazilian army barracks. To enhance security, I implemented Bcrypt for password hashing and express-session for secure session management. Additionally, I utilized Redis to optimize performance by storing session data in memory.
<p>

<br>

<a href="https://github.com/Angelo-Andrade/React_Aroma_Sabor" target="_blank">
  Aroma&Sabor
</a>

<p>
  This was my first JavaScript project, a menu for a coffee shop. It was implemented using Firebase to host the system and the XML database.
  <br>
  <a href="https://project2webcoffee.web.app/" target="_blank">project2webcoffee.web.app/</a>
</p>

<br>

<a href="https://github.com/Angelo-Andrade/C-Registro" target="_blank">
  C Registro
</a>
  
<p>
  My first project, C Registro, is a console app designed to track package traffic in a store. It used C and local .txt files for data storage.
</p>

<br>

<a href="https://github.com/Angelo-Andrade/C-Sharp-Registro" target="_blank">
  C# Registro
</a>

<p>
  This project was a remake of "C Registro," using C# and a MySQL database. It showcases my development in database management and programming languages.
</p>

<br>

---

### Contact:
<div class="contact">
  <a href = "mailto:henrique.a.c.andrade@gmail.com" target="_blank" rel="noopener noreferrer">
    <img src="https://img.shields.io/badge/-Gmail-%23333?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail">
  </a>
  <a href="https://www.linkedin.com/in/henrique-andrade-344231272/" target="_blank" rel="noopener noreferrer">
    <img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn">
  </a>
</div>

---

### Github and Tecnologies stats:
<div>
  <a href="https://github.com/Angelo-Andrade" target="_blank">
    <img width="49%" height="195px" src="https://github-readme-stats.vercel.app/api?username=Angelo-Andrade&show_icons=true&count_private=true&hide_border=true&title_color=c71585&icon_color=da70d6&text_color=c9d1d9&bg_color=0d1117" alt="Henrique Ângelo github stats" />
    <img width="41%" height="195px" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Angelo-Andrade&layout=compact&hide_border=true&title_color=c71585&text_color=ff91a4&bg_color=0d1117" />
  </a>
</div>

---

<img width=100% src="https://capsule-render.vercel.app/api?type=waving&color=b703bc&height=120&section=footer"/>
